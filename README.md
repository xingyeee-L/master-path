...
### ⏳ 沉浸式番茄钟（每张任务卡）
- **一键进入**：每张任务卡支持启动专注计时，全屏沉浸式界面。
- **动态计时**：动态时钟显示 + 环形进度动画，支持最多 +20% 加时。
- **音乐控制**：专注界面内可暂停/继续、切歌，与全局 BGM 无缝协作。
- **自动结算**：结束后自动完成任务并结算 XP，也支持计时中“提前完成”。
- **防作弊**：检测应用切换/后台/系统挂起等中断行为，自动暂停并记录中断次数。

### 🧘 15分钟放空仪式
- **每日必修**：内置“15分钟放空”专属卡片，每日仅限一次。
- **冥想倒计时**：全屏沉浸式倒计时，帮助你切断多巴胺成瘾回路（同样具备加时/中断检测）。
- **动态奖励**：奖励随专注完成度动态结算（低于 80% 不得经验，100% 以上获得 1.5 倍经验）。
...
本项目使用 GitHub Actions 自动构建发布。当你推送 `v*` 标签时（如 `v0.1.3`），CI 将自动打包 macOS 与 Windows 版本并发布到 Releases。
